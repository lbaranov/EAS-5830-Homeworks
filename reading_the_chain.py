...
def is_ordered_block(w3, block_num):
	...
	# Compute the priority fee for each transaction
	fees = []
	for tx in block.transactions:
		...
		if hasattr(tx, "maxPriorityFeePerGas") and tx.maxPriorityFeePerGas is not None:
			...
			# Type 0 transaction
			prio = tx.gasPrice - base_fee
		fees.append(prio)

	# Check that fees[i] >= fees[i+1] for all adjacent pairs
...
if __name__ == "__main__":
	...
	print("Merkle root on-chain:", onchain_root)
	print("Admin has DEFAULT_ADMIN_ROLE?", has_role)
	print("Prime owned by owner_address:", prime)